...
let settings = {
    ...
    sequence: 'HAPPY|BIRTHDAY|TO|RODEEYAH|❤',
...
const musicOptions = [
    ...
    { value: './music/happybirtday_uia.mp3', label: 'Happy Birthday (UIA Cat)' },
    ...
    { value: './music/lambantraianhe.mp3', label: 'Be your boyfriend' },
    { value: './music/denbenanh.mp3', label: 'Come to me' },
    { value: './music/anhnangcuaanh.mp3', label: 'Your sunshine' },
    { value: './music/dunglamtraitimanhdau.mp3', label: "Don't hurt my heart" },
    ...
    { value: './music/coem.mp3', label: 'You are here' },
    { value: './music/emnay.mp3', label: 'Hey you' },
    { value: './music/emlacogiaovungcao.mp3', label: 'You are a highland teacher' },
    { value: './music/emlacogiaovungcaoremix.mp3', label: 'You are a highland teacher (Remix)' },
    { value: './music/lathuguithay.mp3', label: 'A letter to teacher' },
    { value: './music/loithayco.mp3', label: 'Teacher words' },
    { value: './music/nguoithay.mp3', label: 'The teacher' },
    { value: './music/nguoigieomamxanh.mp3', label: 'Green seed sower' },
    { value: './music/nguoidautien.mp3', label: 'The first one' },
    { value: './music/motdoi.mp3', label: 'A lifetime' },
    { value: './music/minhcungnhaudongbang.mp3', label: 'We freeze together' },
    { value: './music/neunhucohaitraitim.mp3', label: 'If there were two hearts' },
    { value: './music/ngayxuaay.mp3', label: 'That old day' },
...
function getIdlePreviewMessage() {
    ...
    return label ? `Selected: ${label}` : 'Choose a song, then press "Preview"';
}
...
function handleMusicPreview() {
    ...
    if (!musicSelect || !musicSelect.value) {
        setMusicPreviewState({
            message: 'Please choose a song before previewing it',
            isPlaying: false
        });
        return;
    }
    ...
    musicPreviewAudio.play().then(() => {
        ...
        console.error('Unable to play music preview:', error);
        stopMusicPreview('Unable to play the preview. Please try again.');
    });
}
...
if (languageSwitchBtn) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            throw new Error(result.error || 'Unable to load data from the server');
        }
...
applySettingsButton.addEventListener('click', () => {
    ...
    if (isBookEnabled) {
        if (settings.pages.length === 0) {
            alert('❌ The book needs pages!\n\nPlease add at least 1 page to the book or turn off the book feature.');
            return;
        }
