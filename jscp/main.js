...
`).join('');
            ...
            voucherList.innerHTML = '<span style="color:#e53935;">Unable to load vouchers!</span>';
        }
    }
    ...
    async handleActionButton() {
        ...
        try {
            ...
            if (this.currentTotal > 0 && this.currentTotal < 10000 && selectedPaymentMethod !== 'PAYPAL') {
                this.showNotification('❌ The minimum payment is 10,000 VND!', 'error');
        ...
        try {
            ...
            if (settingsToUse.enableBook) {
                ...
                if (totalPages > 1 && totalPages % 2 === 0) {
                    this.showNotification(`❌ Invalid page structure! There are currently ${totalPages} pages. Add or remove 1 page to create a valid structure.`, 'error');
                    return;
                }
            }
            ...
            if (!websiteResult.success) {
                throw new Error(websiteResult.error || 'Unable to create website');
            }
        ...
        try {
            ...
            if (this.selectedVoucher) {
                ...
                try {
                    ...
                    this.showNotification('⚠️ Voucher application failed, continuing with the original price', 'warning');
                }
            }
    ...
    async applyVoucher(orderCode) {
        try {
            ...
            if (!voucherData.success) {
                throw new Error(voucherData.message || 'Voucher application failed!');
            }
    ...
    async processPayment(orderCode, amount, websiteId, paymentMethod = 'PAYOS') {
        try {
            ...
            this.showNotification(`🔄 Redirecting to the ${paymentMethodText} payment page...`, 'info');
        ...
        return new Promise((resolve, reject) => {
                ...
                const paymentStatusHandler = (data) => {
                    if (String(data.orderCode) === String(orderCode)) {
                        ...
                        if (data.status === 'PAID') {
                            ...
                            window.cleanupPaymentState(orderCode, false);
                            paymentModal.style.display = 'none';
                            this.showNotification('Payment was canceled', 'warning');
                            window.socketManager.off('payment_status_update', paymentStatusHandler);
            ...
            setTimeout(() => {
                if (!wsMessageReceived) {
                    window.cleanupPaymentState(orderCode, false);
                    this.showNotification('Timeout - Unable to determine payment status!', 'warning');
                    reject(new Error('Timeout - Unable to determine payment status!'));
                }
            }, 300000); // 5 phút
        });
    }
    ...
    showPayPalPayment(checkoutUrl, websiteId, amount) {
        ...
        this.showNotification('🔄 Redirecting to the PayPal payment page...', 'info');
    ...
    <div style="margin-top: 22px; padding: 14px 10px 8px 10px; background: linear-gradient(90deg,#f9fafc,#f3e7fa 70%); border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); text-align: center;">
        <span style="font-size: 15px; color: #222; font-weight: 600;">
            Made For Rodeeyah
        </span>
    </div>
